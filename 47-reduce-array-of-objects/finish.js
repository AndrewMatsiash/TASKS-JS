...
const popularPostsIds = (posts, minimalCommentsQty) => {
  //solution 1
  // return posts.filter(({ comments }) => comments >= minimalCommentsQty).map(({ postId }) => postId);

  //solution 2
  return posts.reduce((acc, { comments, postId }) => {
    return comments >= minimalCommentsQty ? [...acc, postId] : acc;
  }, []);
};
