...
const arraySortInfo = (arr) => {
  const isEveryNumber = arr.every((el) => typeof el === "number");
  const isSortByAscending = arr.every((el, i) => (i > 0 ? el >= arr[i - 1] : true));
  const isSortByDescending = arr.every((el, i) => (i > 0 ? el <= arr[i - 1] : true));
